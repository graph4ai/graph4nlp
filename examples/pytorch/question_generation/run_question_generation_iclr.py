...
class QGModel(nn.Module):
    def __init__(self, vocab, config):
        ...
        # replace the default embedding construction layer with the customized passage-answer alignment embedding construction layer
        ...
        self.g2s.graph_topology.embedding_layer = FusedEmbeddingConstruction(self.vocab.in_word_vocab,
                                        ...
                                        bert_dropout=config['graph_construction_args']['node_embedding'].get('bert_dropout', None),
...
class ModelHandler:
    ...
    def train(self):
        for epoch in range(self.config['epochs']):
            ...
            if epoch > 0 and self.stopper.step(val_scores[self.config['early_stop_metric']], self.model):
                break
