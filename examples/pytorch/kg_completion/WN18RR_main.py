...
def preprocess(dataset_name, delete_data=False):
    ...
    train_path = "examples/pytorch/kg_completion/data/{0}/e1rel_to_e2_train.json".format(
        dataset_name
    )
    dev_ranking_path = (
    ...
    for path, name in zip(
        [train_path, dev_ranking_path, test_ranking_path], ["train", "dev_ranking", "test_ranking"]
    ):
        d.set_path(path)
        p.clear_processors()
        p.add_sent_processor(ToLower())
        p.add_sent_processor(
...
def main(args, model_path):
    ...
    if args.preprocess:
        ...
        torch.save(
            KG_graph, "examples/pytorch/kg_completion/{}/processed/KG_graph.pt".format(args.data)
        )
        return
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            if epoch % 2 == 0 and epoch > 0:
                dev_mrr = ranking_and_hits(
                    model, dev_rank_batcher, vocab, "dev_evaluation", kg_graph=KG_graph
                )
                if dev_mrr > best_mrr:
                    best_mrr = dev_mrr
            ...
            if epoch % 2 == 0:
                if epoch > 0:
                    ranking_and_hits(
                        model, test_rank_batcher, vocab, "test_evaluation", kg_graph=KG_graph
                    )
