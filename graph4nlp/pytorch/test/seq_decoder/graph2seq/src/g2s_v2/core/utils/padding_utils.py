...
def pad_4d_vals(in_vals, dim1_size, dim2_size, dim3_size, dim4_size, fills=0, dtype=np.int32):
    out_val = np.ones((dim1_size, dim2_size, dim3_size, dim4_size), dtype=dtype) * fills
    if dim1_size > len(in_vals): dim1_size = len(in_vals)
    for i in range(dim1_size):
        in_vals_i = in_vals[i]
        cur_dim2_size = dim2_size
...
def pad_target_labels(in_val, max_length, fills=0, dtype=np.float32):
    batch_size = len(in_val)
    out_val = np.ones((batch_size, max_length), dtype=dtype) * fills
