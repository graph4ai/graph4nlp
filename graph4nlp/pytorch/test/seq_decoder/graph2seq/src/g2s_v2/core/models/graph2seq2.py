...
class Graph2Seq(nn.Module):
    ...
    def __init__(self, config, word_embedding, word_vocab):
        ...
        if not self.no_rnn_encoder_on_graph:
            self.graph_rnn_encoder = EncoderRNN(config['word_embed_dim'], enc_hidden_size,
                                                bidirectional=config['enc_bidi'],
                                                num_layers=config['num_enc_rnn_layers'], rnn_type=self.rnn_type,
        ...
        if self.node_encoder == 'rnn':
            self.node_rnn_encoder = EncoderRNN(config['word_embed_dim'], enc_hidden_size,
                                               bidirectional=config['enc_bidi'],
                                               num_layers=config['num_enc_rnn_layers'], rnn_type=self.rnn_type,
                                               rnn_dropout=config['enc_rnn_dropout'], device=self.device)
        ...
        self.decoder = DecoderRNN(self.vocab_size, config['word_embed_dim'], dec_hidden_size, rnn_type=self.rnn_type,
                                  ...
                                  separate_attn=config.get('separate_attn', False),
    ...
    def forward(self, ex, target_tensor=None, criterion=None, criterion_reduction=True, criterion_nll_only=False, \
        ...
        input_mask_transform = torch.zeros(*(input_mask.shape)).to(self.device)

        input_mask_transform[input_mask == 0] = -1
        input_mask_transform[input_mask == 1] = 0

        decoder_outs, enc_attn_weights, coverage_vectors = self.graph4nlp_decoder._run_forward_pass(graph_node_embedding=encoder_outputs.transpose(0, 1),
                                                 graph_node_mask=input_mask_transform,
                                                 rnn_node_embedding=init_encoder_outputs.transpose(0, 1),
                                                 graph_level_embedding=graph_embedding.squeeze(0),
                                                 graph_edge_embedding=None,
                                                 graph_edge_mask=None,
                                                 tgt_seq=target_tensor.transpose(0, 1) if target_tensor is not None else None,
                                                 src_seq=self.filter_oov(input_graphs['g_oov_idx'] if self.pointer and self.copy_node else input_tensor.squeeze(-1), ext_vocab_size))
        # decoder_outs = []
        ...
        #                      encoder_word_idx=input_graphs[
        #                          'g_oov_idx'] if self.pointer and self.copy_node else input_tensor.squeeze(-1),
        ...
        #
        #     decoder_outs.append(decoder_output.unsqueeze(1))
        #
        # decoder_outs = torch.cat(decoder_outs, dim=1)
        predict_logits = torch.log(decoder_outs + VERY_SMALL_NUMBER)

        r.decoded_tokens = predict_logits.argmax(dim=-1).transpose(0, 1).detach()

        loss = 0
        if criterion:
            use_coverage = len(coverage_vectors) != 0
            for i in range(target_length):

                pred_loss = criterion(predict_logits[:, i, :].view(-1, predict_logits.shape[-1]),
                                       target_tensor[i, :].view(-1))
                loss += pred_loss

                if use_coverage and coverage_vectors[i] is not None:
                    coverage_loss = torch.sum(torch.min(coverage_vectors[i], enc_attn_weights[i])) / batch_size * self.cover_loss
                    loss += coverage_loss
            r.loss = loss
            r.loss_value = loss.item()
            # print(r.loss)
        return r
