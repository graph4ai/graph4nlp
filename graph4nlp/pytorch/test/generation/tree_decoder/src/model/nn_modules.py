...
class SparseUniformNeighborSampler(object):
    def __init__(self, adj,):
        assert sparse.issparse(
            adj), "SparseUniformNeighborSampler: not sparse.issparse(adj)"
        self.adj = adj
...
#         self.fc_x = nn.Linear(input_dim/2, output_dim/2, bias=False)
#         self.fc_neib = nn.Linear(input_dim/2, output_dim/2, bias=False)
