...
class Hypothesis(object):
    def __init__(self, tokens, log_probs, dec_state, num_non_words, enc_attn_weights_gcn, enc_attn_weights_gmp, use_coverage):
        self.tokens = tokens
        self.log_probs = log_probs
        self.dec_state = dec_state
        self.num_non_words = num_non_words
        self.enc_attn_weights_gcn = enc_attn_weights_gcn
        self.enc_attn_weights_gmp = enc_attn_weights_gmp
        self.use_coverage = use_coverage
        if not self.use_coverage:
            self.enc_attn_weights_gcn = []
            self.enc_attn_weights_gmp = []
    ...
    def create_next(self, token, log_prob, dec_state, non_word, add_enc_attn_weights):
        if self.use_coverage:
            add_enc_attn_weights_gcn, add_enc_attn_weights_gmp = add_enc_attn_weights
            assert len(add_enc_attn_weights_gcn.shape) == 3
            assert add_enc_attn_weights_gcn.shape[0] == 1
            assert add_enc_attn_weights_gcn.shape[1] == 1

            enc_attn_weights_processed_gcn = [t.clone() for t in self.enc_attn_weights_gcn]
            enc_attn_weights_processed_gcn.append(add_enc_attn_weights_gcn)

            enc_attn_weights_processed_gmp = [t.clone() for t in self.enc_attn_weights_gmp]
            enc_attn_weights_processed_gmp.append(add_enc_attn_weights_gmp)
        else:
            enc_attn_weights_processed_gcn = []
            enc_attn_weights_processed_gmp = []
        ...
        return Hypothesis(tokens=self.tokens + [token], log_probs=self.log_probs + [log_prob],
                          ...
                          enc_attn_weights_gcn=enc_attn_weights_processed_gcn,
                          enc_attn_weights_gmp=enc_attn_weights_processed_gmp,
...
class DecoderStrategy(StrategyBase):
    ...
    def _beam_search(self,
                     ...
                     gmp_node_embedding=None,
                     gmp_emb_mask=None,
                     graph_level_embedding=None,
                     gmp_level_embedding=None,
        ...
        combine_init_emb = graph_level_embedding + gmp_level_embedding
        # combine_init_emb = gmp_level_embedding
        # combine_init_emb = graph_level_embedding
        # combine_init_emb = None

        decoder_state = self.decoder.get_decoder_init_state(rnn_type=self.rnn_type, batch_size=batch_size,
                                                            content=combine_init_emb)
        ...
        for batch_idx in range(batch_size):
            ...
            single_gmp_node_embedding = gmp_node_embedding[batch_idx, :, :].unsqueeze(
                0).expand(beam_size, -1, -1).contiguous() if gmp_node_embedding is not None else None
            single_gmp_emb_mask = gmp_emb_mask[batch_idx, :].unsqueeze(
                0).expand(beam_size, -1).contiguous() if gmp_emb_mask is not None else None
            ...
            while len(hypos) > 0 and step <= self.max_decoder_step:
                ...
                if step > 0 and self.use_coverage:
                    for ii in range(step):
                        coverage_input_gcn.append(torch.cat([h.enc_attn_weights_gcn[ii] for h in hypos], dim=1))
                        coverage_input_gmp.append(torch.cat([h.enc_attn_weights_gmp[ii] for h in hypos], dim=1))

                decoder_output, single_decoder_state, dec_attn_scores, _ = \
                    self.decoder.decode_step(decoder_input=decoder_input,
                                             rnn_state=single_decoder_state,
                                             gnn_dec_input_mask=single_graph_node_mask,
                                             gnn_encoder_out=single_graph_node_embedding,
                                             gmp_encoder_out=single_gmp_node_embedding,
                                             gmp_emb_mask=single_gmp_emb_mask,
                                             enc_attn_weights_average_gcn=coverage_input_gcn,
                                             enc_attn_weights_average_gmp=coverage_input_gmp,
                ...
                for in_idx in range(n_hypos):
                    for out_idx in range(beam_size):
                        ...
                        if self.use_coverage:
                            new_enc_attn_weights_gcn = dec_attn_scores[0][in_idx, :].unsqueeze(0).unsqueeze(0)
                            new_enc_attn_weights_gmp = dec_attn_scores[1][in_idx, :].unsqueeze(0).unsqueeze(0)
                            new_enc_attn_weights = (new_enc_attn_weights_gcn, new_enc_attn_weights_gmp)
                        else:
                            new_enc_attn_weights = None
