...
class GMP2Seq(Graph2XBase):
    """
        ...
        >>> graph2seq = GMP2Seq.from_args(opt=opt, vocab_model=vocab_model, device=torch.device("cuda:0"))
    ...
    def __init__(self, vocab_model, emb_input_size, emb_hidden_size, embedding_style,
                 ...
                 **kwargs):
        super(GMP2Seq, self).__init__(vocab_model=vocab_model, emb_input_size=emb_input_size, emb_hidden_size=emb_hidden_size,
    ...
    def encoder_decoder(self, batch_graph, old_graph_list, gmp_seq, gmp_jump,
                        oov_dict=None, tgt_seq=None):
        # run GNN
        # batch_graph = self.gnn_encoder(batch_graph)

        # run GMP
        gmp_outputs, gmp_encoder_feature, (gmp_h_max, gmp_c_max), gmp_masks = self.gmp_encoder(gmp_seq, gmp_jump)
        gmp_level_emb = torch.cat([gmp_h_max[0],gmp_h_max[1]], dim=-1)
        prob, enc_attn_weights, coverage_vectors = self.seq_decoder._run_forward_pass(gmp_outputs,
                                                                                      src_seq=gmp_seq,
                                                                                      tgt_seq=tgt_seq,
                                                                                      graph_node_mask=gmp_masks,
                                                                                      graph_level_embedding=gmp_level_emb,
    ...
    def encoder_decoder_beam_search(self, batch_graph, old_graph_list, gmp_seq, gmp_jump, beam_size, topk=1, oov_dict=None):
        ...
        beam_results = generator._beam_search(gmp_outputs, graph_node_mask=gmp_masks, graph_level_embedding=gmp_level_emb, src_seq=gmp_seq, oov_dict=oov_dict, topk=topk, beam_size=beam_size)
        return beam_results
